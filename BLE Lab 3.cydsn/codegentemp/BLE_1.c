...
{
    { 0x02u, 0x01u, 0x06u, 0x0Bu, 0x09u, 0x43u, 0x68u,
    0x72u, 0x69u, 0x73u, 0x5Fu, 0x45u, 0x76u, 0x61u, 0x6Eu,
    0x05u, 0x03u, 0x0Du, 0x18u, 0x0Au, 0x18u, 0x00u, 0x00u,
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u }, /* uint8 advertising_data[CYBLE_MAX_ADV_DATA_LEN] */
    0x15u,      /* uint8 adv_data_length */
};

CYBLE_GAPP_SCAN_RSP_DATA_T cyBle_scanRspData =
{
    { 0x0Bu, 0x09u, 0x43u, 0x68u, 0x72u, 0x69u, 0x73u,
    0x5Fu, 0x45u, 0x76u, 0x61u, 0x6Eu, 0x00u, 0x00u, 0x00u,
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u,
    0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u, 0x00u }, /* uint8 scan_rsp_data[CYBLE_MAX_SCAN_RSP_DATA_LEN] */
    0x0Cu,      /* uint8 scan_rsp_data_length */
};
...
CYBLE_API_RESULT_T CyBle_StoreBondingData(uint8 isForceWrite)
{
    ...
    *******************************************************************************
    ...
    Return:
     ...
     failed. Following are the possible error codes.
     <table>
      Errors codes                        Values  Description
      ------------                        ------  -----------
      CYBLE_ERROR_OK                      0x0000  On successful operation.
      CYBLE_ERROR_STACK_INTERNAL          0x0003  An error occurred in the 
                                                   BLE stack.
     </table>
    ...
    *******************************************************************************
    ...
    Return:
     ...
     <table>
      ...
      ------------                        -----------
      CYBLE_ERROR_OK                      On successful operation.
      CYBLE_ERROR_STACK_INTERNAL          On error occurred in the 
                                           BLE stack.
      CYBLE_ERROR_INVALID_PARAMETER       On specifying NULL as input
                                           parameter for 'scanInfo' or if
                                           any element with in 'scanInfo'
        ...
        ******************************************************************************/
        static void CyBle_ChangeAdLocalName(const char8 name[], uint8 dest)
        {
            ...
            while((byteCounter < maxLength) && (0u == flag))
            {
                ...
                {
                    /* The end of advertisement data structure was encountered though exit the 
                      loop. */
                    break;
                }
            }
    ...
    ******************************************************************************/
    CYBLE_API_RESULT_T CyBle_GapSetLocalName(const char8 name[])
    {
        ...
        if(NULL != name)
        {
            ...
            ptr = (char8 *)
                cyBle_gattDB[CYBLE_GAP_DEVICE_NAME_INDEX].attValue.attFormatValue.attGenericVal;
            ...
            /* First need to get the length of the characteristic data in the GATT
                ...
                be used. */
            charLen = (uint8) cyBle_gattDB[CYBLE_GAP_DEVICE_NAME_INDEX].attValue.attFormatValue.length;

            /* Copy name into characteristic */
            for(i = 0u; ((i < (charLen - 1u)) && (CYBLE_NULL_CHARCTER != name[i])); i++)
            {
                ptr[i] = name[i];
            }
    ...
    ******************************************************************************/
    CYBLE_API_RESULT_T CyBle_GapGetLocalName(char8 name[])
    {
        ...
        {
            /* Get the pointer to the Device Name characteristic  */
            ptr = (char8 *) cyBle_gattDB[CYBLE_GAP_DEVICE_NAME_INDEX].attValue.attFormatValue.attGenericVal;
            ...
            for(i = 0u; ((i < (charLen - 1u)) && (CYBLE_NULL_CHARCTER != ptr[i])); i++)
            {
                name[i] = ptr[i];
            }
